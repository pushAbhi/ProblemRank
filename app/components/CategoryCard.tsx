...
export function CategoryCard({ icon: Icon, label, count, tone, active }: CategoryCardProps) {
  return (
    <button
      className={`group flex min-w-45 items-center gap-3 rounded-2xl bg-card p-3 text-left shadow-lg border border-black/20 transition hover:-translate-y-0.5 hover:border-brand/40 ${
        active ? "border-brand/60 ring-2 ring-brand/15" : "border-border"
      }`}
    >
